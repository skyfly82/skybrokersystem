...
class Customer extends Model
{
    ...
    protected $fillable = [
        'uuid', 'type', 'company_name', 'company_short_name', 'nip', 'regon', 'krs',
        ...
        'google_id', 'facebook_id', 'linkedin_id', 'social_avatar', 'registration_source',
    ...
    {
        if ($this->isCompany()) {
            return $this->company_name ?? 'Firma';
        }
        
        return trim(($this->individual_first_name ?? '') . ' ' . ($this->individual_last_name ?? '')) ?: 'Klient Indywidualny';
    }
    ...
    public function getSocialAvatar(string $provider = null): ?string
    {
        $avatars = $this->social_avatar ?? [];
        
        if ($provider && isset($avatars[$provider])) {
            return $avatars[$provider];
        }
        
        // Return first available avatar
        return !empty($avatars) ? reset($avatars) : null;
    }

    public function setSocialAvatar(string $provider, string $avatarUrl): void
    {
        $avatars = $this->social_avatar ?? [];
        $avatars[$provider] = $avatarUrl;
        $this->update(['social_avatar' => $avatars]);
    }

    public function isSocialRegistration(): bool
    {
        return in_array($this->registration_source, ['google', 'facebook', 'linkedin']);
    }
}
